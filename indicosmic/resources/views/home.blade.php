...
@if($name=="")
{{"name is empty"}}
@elseif($name=="Indicosmic")
{{"(:name is correct :)"}}
@else
{{"name is incorrect"}}
@endif
@unless($name=="Indicosmic")
This is not Indicosmic 
@endunless
...
<div class="container">
@for($i=1; $i<=5; $i++)
        <!-- {{--if the value of i is 5 so we will break the loop --}} -->
        @if($i==5)
            @break
        @else
            {{ $i }}
